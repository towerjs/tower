import { resolveTowerService } from './runtime.js'
...
export function createLazyModule<T>(moduleName: string): T {
  const proxy = new Proxy(function () {} as any, {
    get(_, prop) {
      if (prop === 'then') {
        return (onFulfilled: ((v: any) => any) | undefined, onRejected: ((e: any) => any) | undefined) =>
          resolveTowerService<T>(moduleName).then(onFulfilled, onRejected)
      }
      ...
      return (...args: unknown[]) =>
        resolveTowerService<any>(moduleName).then((service) => {
          const value = service[prop]
          if (typeof value !== 'function') {
            if (args.length > 0) throw new TypeError(`${moduleName}.${String(prop)} is not a function`)
            return value
          }
    ...
    apply(_, __, args) {
      return resolveTowerService<any>(moduleName).then((service) => {
