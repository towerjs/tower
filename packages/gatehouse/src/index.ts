...
export function gatehouse(config: GatehouseConfig): TowerModule & GatehouseModule {
  ...
  return {
    ...
    phone: {
      otp: {
        async send(params: PhoneOtpSendParams) {
          return adapter!.sendPhoneOTPModule(params);
        },
        async confirm(params: PhoneOtpConfirmParams) {
          return adapter!.verifyPhoneOTPModule(params);
        },
      },
    },
    ...
    async can(params: { user: GatehouseUser; permission: string | string[]; organizationId?: string }) {
