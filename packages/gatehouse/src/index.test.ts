...
const mocks = vi.hoisted(() => {
  ...
  return {
    alsStore,
    mockBetterAuth, mockToNextJsHandler, mockRegisterModule,
    mockSignInEmail, mockSignUpEmail, mockSignOut,
    mockGetSession, mockListSessions,
    mockCreateOrganization, mockListOrganizations,
    reset,
  }
})
...
vi.mock("@towerjs/blueprint", () => ({
  towerContext: {
    ...
    run: vi.fn(async (ctx: any, handler: () => any) => {
      ...
      finally { Object.assign(mocks.alsStore, prev) }
    }),
  },
...
describe("buildProxiedApi", () => {
  it("wraps method to inject headers", async () => {
    ...
    expect(inner).toHaveBeenCalledWith(
      expect.objectContaining({ headers, body: { email: "a@b.com" } }),
    )
  })
  ...
  it("passes GET-method params as query", async () => {
    ...
    expect(inner).toHaveBeenCalledWith(
      expect.objectContaining({ query: { userId: "1" } }),
    )
  })
  ...
  it("allows raw params via body/query/headers keys", async () => {
    ...
    expect(inner).toHaveBeenCalledWith(
      expect.objectContaining({ body: { raw: true } }),
    )
  })
...
describe("BetterAuthAdapter", () => {
  ...
  it("proxy allows authenticated users through", async () => {
    ...
    mocks.mockGetSession.mockResolvedValue({
      user: { id: "u1", name: "A", email: "a@b.com", emailVerified: true, image: null, createdAt: new Date(), updatedAt: new Date() },
      session: { id: "s1", userId: "u1", expiresAt: new Date(), token: "tok" },
    })
...
describe("social provider expansion", () => {
  ...
  it("throws for missing provider credentials", async () => {
    const { BetterAuthAdapter } = await import("./providers/better-auth.js")
    expect(() => new (BetterAuthAdapter as any)({
      provider: "better-auth",
      social: ["github"],
    } as any, {})).toThrow("Missing credentials")
  })
