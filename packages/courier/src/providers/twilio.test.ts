...
describe("TwilioSmsProvider", () => {
  describe("constructor", () => {
    it("uses credentials from config", () => {
      const p = new TwilioSmsProvider({ provider: "twilio", accountSid: "ACxxx", authToken: "tok", messagingServiceSid: "MGxxx" })
    ...
    it("throws when authToken missing", () => {
      expect(() => new TwilioSmsProvider({ provider: "twilio", accountSid: "ACx", messagingServiceSid: "MGx" })).toThrow(
        "[courier.sms] Missing Twilio credentials",
      )
    })
  })
  ...
  describe("send", () => {
    it("sends SMS with messagingServiceSid and body", async () => {
      const p = new TwilioSmsProvider({ provider: "twilio", accountSid: "ACx", authToken: "tok", messagingServiceSid: "MGx" })
    ...
    it("uses from when messagingServiceSid not set", async () => {
      ...
      expect(mockMessagesCreate).toHaveBeenCalledWith(
        expect.objectContaining({ from: "+1987654321" }),
      )
    })
    ...
    it("throws when messagingServiceSid and from both missing", async () => {
      ...
      await expect(p.send({ to: "+1234567890", body: "Hello" })).rejects.toThrow(
        "[courier.sms] Missing sender",
      )
    })
    ...
    it("throws on Twilio API error", async () => {
      ...
      await expect(
        p.send({ to: "+1234567890", body: "Hello" }),
      ).rejects.toThrow("Twilio error: 21610 - Unreachable")
    })
  })
})
