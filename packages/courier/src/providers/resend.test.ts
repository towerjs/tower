...
describe('ResendEmailProvider', () => {
  ...
  describe('send', () => {
    it('uses from from config when params omit it', async () => {
      ...
      expect(mockResendCreate).toHaveBeenCalledWith(expect.objectContaining({ from: 'config@example.com' }))
    })
    ...
    it('uses from from params', async () => {
      ...
      expect(mockResendCreate).toHaveBeenCalledWith(expect.objectContaining({ from: 'custom@example.com' }))
    })
    ...
    it('throws on Resend error', async () => {
      ...
      await expect(p.send({ to: 'a@b.com', subject: 'hi', text: 'world' })).rejects.toThrow(
        '[courier.email] Rate limited'
      )
    })
    ...
    it('passes attachments', async () => {
      ...
      await p.send({
        ...
        subject: 's',
        text: 'body',
        attachments: [{ filename: 'file.pdf', content: Buffer.from('pdf data'), contentType: 'application/pdf' }],
      })
      expect(mockResendCreate).toHaveBeenCalledWith(
        expect.objectContaining({
