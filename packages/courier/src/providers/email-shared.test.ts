...
vi.mock("@react-email/render", () => ({
  render: vi.fn(async (el: ReactElement) =>
    `<html>${JSON.stringify(el)}</html>`,
  ),
...
describe("resolveEmailContent", () => {
  ...
  it("prefers explicit html over react template", async () => {
    const result = await resolveEmailContent({ to: "a@b.com", subject: "x", html: "<p>explicit</p>", react: h("Hello") })
