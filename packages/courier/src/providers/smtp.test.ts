...
describe("SmtpEmailProvider", () => {
  ...
  describe("send", () => {
    ...
    it("throws when from missing", async () => {
      const p = new SmtpEmailProvider({ provider: "smtp", host: "smtp.example.com", port: 587 })
      await expect(
        p.send({ to: "a@b.com", subject: "hi", text: "hello" }),
      ).rejects.toThrow("[courier.email] Missing from address")
    })

    it("uses from from config", async () => {
      const p = new SmtpEmailProvider({ provider: "smtp", host: "smtp.example.com", port: 587, from: "config@example.com" })
    ...
    it("passes attachments", async () => {
      ...
      await p.send({
        ...
        text: "body",
        attachments: [
          { filename: "f.pdf", content: Buffer.from("data"), contentType: "application/pdf" },
        ],
      })
