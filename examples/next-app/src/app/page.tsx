  ...
  return (
    <main className="mx-auto max-w-3xl p-8 space-y-6">
      ...
      <p className="text-neutral-600">
        Courier is configured with a provider-agnostic API. Swap providers in{" "}
        <code>tower.config.ts</code> and keep app code unchanged.
      </p>

      <section className="space-y-3">
        <h2 className="text-xl font-medium">Try Courier API</h2>
        <pre className="rounded bg-neutral-100 p-4 text-sm overflow-x-auto">{`curl -X POST http://localhost:3000/api/courier/email \\
...
curl -X POST http://localhost:3000/api/courier/sms \\
  -H "content-type: application/json" \\
  -d '{"to":"+15551234567","body":"Hello from Tower"}'

curl -X POST http://localhost:3000/api/courier/push \\
  -H "content-type: application/json" \\
  -d '{"subscription":{"endpoint":"...","keys":{"p256dh":"...","auth":"..."}}}'`}</pre>
      ...
      <section className="space-y-3">
        ...
        <p className="text-neutral-600">
          When Courier is enabled, Gatehouse automatically routes built-in auth
